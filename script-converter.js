...
- @returns {Object} 调试信息
  */
  function debugScriptContent(content) {
  const debugInfo = {
  contentLength: content.length,
  hasBOM: content.charCodeAt(0) === 0xFEFF,
  firstChars: Array.from(content.slice(0, 20)).map(c => c.charCodeAt(0)),
  hasRewriteLocal: content.includes(’[rewrite_local]’),
  matchResults: {},
  charDump: content.split(’’).map((c, i) => ({index: i, char: c, code: c.charCodeAt(0)})).slice(0, 50)
  };

// 测试各种正则表达式匹配
const regexTests = [
‘rewriteLocal1’, /[rewrite_local]([\s\S]*?)(?=[|$)/i,
‘rewriteLocal2’, /[rewrite_local]([\s\S]*)/i,
‘scriptResponseBody’, /script-response-body/i,
‘urlPattern’, /^https/i
];

for (let i = 0; i < regexTests.length; i += 2) {
const name = regexTests[i];
const regex = regexTests[i+1];
const match = content.match(regex);
debugInfo.matchResults[name] = match ? {
found: true,
index: match.index,
length: match[0].length,
groups: match.length,
content: match[0].slice(0, 30) + (match[0].length > 30 ? ‘…’ : ‘’)
} : {found: false};
}
...
- 安全地提取脚本内容
- @param {string} content 文件内容
- @returns {Object} 提取结果和调试信息
  */
  function safeExtractScriptContent(content) {
  try {
  // 去除可能的BOM和不可见字符
  ...
  return {
  success: false,
  debug: debugScriptContent(content),
...
- @param {string} content 原始脚本内容
- @param {string} targetFormat 目标格式 (loon, surge, quantumultx)
- @param {boolean} debug 是否返回调试信息
- @returns {Object} 转换结果和调试信息
  */
  function convertScriptEnhanced(content, targetFormat, debug = false) {
  try {
  // 预处理：清理BOM和规范化换行符
  content = content.replace(/^\uFEFF/, ‘’);
  content = content.replace(/\r\n/g, ‘\n’).replace(/\r/g, ‘\n’);
  ...
  let result;
  if (targetFormat === ‘loon’) {
  result = convertToLoon(scriptInfo);
  } else if (targetFormat === ‘surge’) {
  result = convertToSurge(scriptInfo);
  } else {
  result = JSON.stringify(scriptInfo, null, 2);
  }
  ...
  debugInfo: debugScriptContent(content)
  } : undefined
  };
  } catch (error) {
  return {
...
- 测试函数 - 用于测试两个不同文件的解析
- @param {string} file1 第一个文件的内容
- @param {string} file2 第二个文件的内容
- @returns {Object} 测试结果
  */
  function testFileParsing(file1, file2) {
  const result = {
  file1: {
  debug: debugScriptContent(file1),
  parsed: null,
  error: null
  },
  file2: {
  debug: debugScriptContent(file2),
  parsed: null,
  error: null
  }
  };
...
try {
// 尝试解析第一个文件
const extracted1 = extractScriptContent(file1);
const parsed1 = parseScript(extracted1);
result.file1.parsed = parsed1;
} catch (error) {
result.file1.error = {
...
try {
// 尝试解析第二个文件
const extracted2 = extractScriptContent(file2);
const parsed2 = parseScript(extracted2);
result.file2.parsed = parsed2;
} catch (error) {
result.file2.error = {
message: error.message,
stack: error.stack
