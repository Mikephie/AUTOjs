 ...
 * 段落支持：[General]/[Rule]/[Map Local]/[URL Rewrite]/[Script]/[MITM]
 * - [General]/[Rule]/[Map Local]/[URL Rewrite]/[Script]：保序不去重
 * - [MITM] hostname 去重+排序
 * - 汇总 [General] 的 force-http-engine-hosts（去重+排序，统一一行）
 * - 无段落模块：整文件视为 [Script]；从注释 "# hostname = ..." 也会收集域名
...
const EXTRA_HOSTNAMES = []; // 可选：全局固定追加
...
function parseSections(text){
  ...
  const out = {
    'General':[], 'Rule':[], 'Map Local':[], 'URL Rewrite':[], 'Script':[], 'MITM':[]
  };
  let current = null;
...
function build(){
  ...
  console.log('[OK] Surge mcollection built:', OUT);
}
