...
function listFiles(dir) {
  ...
  (function walk(d){
    ...
    for (const name of fs.readdirSync(d)) {
      ...
      if (/mcollection/i.test(name)) continue;
      if (extpl.test(name)) out.push(p);
    }
  })(dir);
...
function parseSections(text){
  ...
  const out = { 'General':[], 'Rule':[], 'Map Local':[], 'Rewrite':[], 'Script':[], 'MITM':[] };
  let current = null;
...
function build(){
  ...
  console.log('[Loon] scanning:', SRC_DIR);
  console.log('[Loon] files:', files.length, files.map(p=>path.relative(SRC_DIR,p)));
  ...
  for (const p of files){
    ...
    if (hasAnyContent(sec)){
      ...
      if (sec['Rewrite'].length){
        REWRITE.push('', '#************************************#', `# ${display}`, ...sec['Rewrite']); appended = true;
      }
    ...
    console.log(`[Loon] ${display} -> ${appended ? 'appended' : 'skipped'}`);
  }
